...
var logColumns = "Time,PID,ProcessName,CPU,AvgCPU,UserCPU,SysCPU,VmRSS,VmSize,rcharPerSecond,wcharPerSecond,syscrPerSecond,syscwPerSecond\n";
...
function PUsage (opts) {
  ...
  self.logFile = opts.logFile || null;
  ...
  if (self.logFile && typeof self.logFile !== 'string')
    throw new TypeError('PUsage(): invalid option: logFile must be a string');
  if (self.logStream && (typeof self.logStream !== 'object' || !self.logStream.write))
    throw new TypeError('PUsage(): invalid option: logStream must be a writable stream');

  if (self.logFile)
    self.logStream = fs.createWriteStream(self.logFile);
}
...
function stopLoop(self) {
  ...
  if (self.logFile) self.logStream.end();
  self.forEachProc(function (p) {
    fs.close(p.statFd);
    fs.close(p.statusFd);
    fs.close(p.ioFd);
    if (p.logfile) p.logStream.end();
  });
}
    ...
    getState(self, function (error, state_after) {
      ...
      if (state_before !== null) {
        ...
        self.forEachProc(function (p) {
          ...
          if (p.logStream) p.logStream.write(logLine);
...
PUsage.prototype.start = function () {
  ...
  fs.open('/proc/stat', 'r', function (error, fd) {
    ...
    if (self.logStream)
      self.logStream.write(logColumns);
    loop(self, null);
  });
};
...
PUsage.prototype.watch = function () {
  ...
  if (pid === parseInt(pid, 10)) { // is interger, i.e. PID
    ...
    var logFile = null, logStream = null;
    if (arguments.length > 1) {
      if (typeof arguments[1] === 'string')
        logFile = arguments[1];
      else if (typeof arguments[1] === 'object' && arguments[1].write) // is writeStream
        logStream = arguments[1];
      else
        throw new TypeError("PUsage.watch(): invalid argument #2: expected filename or writeStream.");
    }
    fs.open('/proc/' + pid.toString() + '/stat', 'r', function (error, statFd) {
      if (error) throw error;
      fs.read(statFd, statBuff, 0, 1024, 0, function (error, num, buff) {
        if (error) throw error;
        if (logFile) logStream = fs.createWriteStream(logFile);
        if (logStream) logStream.write(logColumns);
        var proc = {
    ...
    var args = Array.prototype.slice.call(arguments); // copy args
    getPidOf(pname, function (error, pids) {
      ...
      args[0] = pids[pids.length - 1];
      console.log(args);
      self.watch.apply(self, args);
    });
  }
  else
